...
const Match = ({match}) => {
  ...
  const team1 = T1[0]
  const team2 = T2[0]
  return <Link href={`/match/${Eid}`} >
  <Paper elevation={2} sx={{my: 1.5, py: 0.5,  width: '100%', mx: 'auto', maxWidth: '700px', display: 'flex', flexDirection: 'row', borderLeftColor: 'warning.main', borderLeftWidth: 2, borderLeftStyle: 'solid', '&:hover': {cursor: 'pointer'}, bgcolor: 'rgba(0, 51, 102, 0.1)' }}>

    <Typography sx={{color: 'warning.main', fontSize: 15, textAlign: 'center', mx:2, alignSelf: 'center'}}>
      {Eps}
    </Typography>
